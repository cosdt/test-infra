...
def _create_check_run_with_fallback(
    *,
    ...
    try:
        upstream_installation_id = utils.get_repo_installation_id(
            jwt_token, upstream_repo
        )
        token = utils.get_installation_token(jwt_token, upstream_installation_id)
        utils.upsert_completed_check_run(
            check_name, token, upstream_repo, device, commit_sha, conclusion, run_url
        )
        return
...
async def handle_ci_result(req: Request):
    ...
    if level == "L3":
        ...
        try:
            ...
            raise HTTPException(
                status_code=502, detail=f"Failed to get upstream token: {e}"
            )
    ...
    check_name = "gate" if level == "L4" else "info"
