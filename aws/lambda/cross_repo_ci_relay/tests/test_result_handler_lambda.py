...
class TestResultLambdaFunction(unittest.TestCase):
    ...
    def test_invalid_method(self, mock_handler, mock_get_config):
        event = {
    ...
    @patch("result.lambda_function._get_config")
    @patch("result.lambda_function.result_handler")
    def test_invalid_path(self, mock_handler, mock_get_config):
        event = {
    ...
    @patch("result.lambda_function._get_config")
    @patch("result.lambda_function.result_handler")
    def test_invalid_json(self, mock_handler, mock_get_config):
        event = {
    ...
    @patch("result.lambda_function._get_config")
    @patch("result.lambda_function.result_handler")
    def test_http_exception(self, mock_handler, mock_get_config):
        event = {
            "requestContext": {"http": {"method": "POST", "path": "/github/result"}},
            "body": '{"status": "completed"}',
            "isBase64Encoded": False,
        }
        mock_handler.handle.side_effect = HTTPException(401, "Invalid token")
        response = lambda_handler(event, {})
    ...
    @patch("result.lambda_function._get_config")
    @patch("result.lambda_function.result_handler")
    def test_unhandled_exception(self, mock_handler, mock_get_config, mock_logger):
        event = {
            "requestContext": {"http": {"method": "POST", "path": "/github/result"}},
            "body": '{"status": "completed"}',
            "isBase64Encoded": False,
    ...
    @patch("result.lambda_function._get_config")
    @patch("result.lambda_function.result_handler")
    def test_happy_path(self, mock_handler, mock_get_config):
        event = {
            "requestContext": {"http": {"method": "POST", "path": "/github/result"}},
        ...
        mock_handler.handle.assert_called_once_with(
            mock_get_config.return_value, "tok", {"status": "completed"}
        )

    @patch("result.lambda_function._get_config")
    @patch("result.lambda_function.result_handler")
    def test_base64_encoded_body(self, mock_handler, mock_get_config):
        import base64

        body = base64.b64encode(b'{"status": "completed"}').decode("utf-8")
        event = {
            "requestContext": {"http": {"method": "POST", "path": "/github/result"}},
            "headers": {"authorization": "tok"},
            "body": body,
